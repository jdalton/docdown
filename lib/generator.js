...
function generateDoc(source, options) {
  ...
  _.each(api, function(entry) {
    ...
    entryData.entryHref = _.template(entryData.entryHref)(entryData);
    entryData.sourceHref = _.template(entryData.sourceHref)(entryData);
    entryData.tocHref = _.template(entryData.tocHref)(entryData);

    entryData.entryLink = _.template(entryData.entryLink)(entryData);
    entryData.sourceLink = _.template(entryData.sourceLink)(entryData);
    entryData.tocLink = _.template(entryData.tocLink)(entryData);
  ...
  docMarkdown.push(' [1]: #' + _.get(tocGroups, 0, '').toLowerCase() + ' "Jump back to the TOC."\n');
