...
function generateDoc(source, options) {
  ...
  if (byCategories) {
    ...
    var args = [tocGroups];
    args.push.apply(args, specialCategories);
    _.pull.apply(_, args);

    // Sort categories and append special categories.
    tocGroups.sort(utils.compareNatural);
    tocGroups.push.apply(tocGroups, specialCategories);
  }
  else {
    tocGroups.sort(utils.compareNatural);
  }
  ...
  _.each(tocGroups, function(group) {
    ...
    organized[group].sort(function(value, other) {
      return utils.compareNatural(
        value.getMembers(0) + getSeparator(value) + value.getName(),
        other.getMembers(0) + getSeparator(other) + other.getName()
      );
    });
    ...
    _.each(organized[group], function(entry) {
      ...
      if (entry.isAlias()) {
        ...
        markdown.push(
          '* <a href="#' + owner.getHash() + '" class="alias">`' +
          title + '` -> `' + owner.getName() + '`' +
          '</a>'
        );
      } else {
