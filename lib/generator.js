...
function generateDoc(source,options) {
      ...
      if (!anEntry.isAlias()) {
        ...
        if (theArguments.length > 0) {
          ...
          theArguments.forEach(function(anArgument, index) {
            anEntry.markdown.push(
              _.template('${num}. `${name}` *(${type})*: ${desc}')({
                'desc': markdownEscape(anArgument[2]),
                'name': anArgument[1],
                'num':  index + 1,
                'type': markdownEscape(anArgument[0])
              })
            );
        ...
        if (theReturns.length > 0) {
          ...
          anEntry.markdown.push(
            _.template('*(${type})*: ${desc}')({
              'desc': markdownEscape(theReturns[1]),
              'type': markdownEscape(theReturns[0])
            })
          )
          anEntry.markdown.push('');
        }
  ...
  tocGroups.forEach(function(aGroup) {
    ...
    //sort the groups
    organizedEntries[aGroup].sort(utils.compareNatural);
