...
function getAliases(index) {
  if (this._aliases === undefined) {
    var result = getValue(this, 'alias');
    if (result) {
...
function getCategory() {
  ...
  return this.getType() == 'Function' ? 'Methods' : 'Properties';
}
 ...
 */
function getDesc() {
  var result = getValue(this, 'description');
  if (!result) {
    return result;
  }
  result = _.trim(result
...
function getMembers(index) {
  if (this._members === undefined) {
    var result = getValue(this, 'member') || getValue(this, 'memberOf');
    if (result) {
      result = result.split(/,\s*/);
      result.sort(util.compareNatural);
    }
...
function getParams(index) {
  if (this._params === undefined) {
    this._params = _.map(_.filter(this.parsed.tags, ['title', 'param']), function(tag) {
      return [_.get(tag, 'type.name', '*'), _.get(tag, 'name', ''),  _.get(tag, 'description', '')];
    });
  }
