...
function getTag(entry, tagName) {
  ...
  return _.find(parsed.tags, ['title', tagName]);
}
...
function getValue(entry, tagName) {
  ...
  var tag = getTag(entry, tagName);
  if (tagName == 'type') {
    return _.get(tag, 'type.name') || '';
  }
  return _.get(tag, 'name') || _.get(tag, 'description') || '';
}

function hasTag(entry, tagName) {
  return getTag(entry, tagName) !== undefined;
}
...
function reduceParamType(tag) {
  var result = '',
      expression = _.get(tag, 'expression'),
      type = _.get(tag, 'type');
  ...
  switch (type) {
    ...
    case 'TypeApplication':
      ...
      result = _(tag)
        .chain()
        .get('applications')
        .map(_.flow(reduceParamType, fp.add(fp, '[]')))
        .sort(util.compareNatural)
        .join('|')
        .value();
      break;
  ...
  if (expression) {
    result += reduceParamType(expression);
  }
  if (type == 'UnionType') {
    result = '(' + result + ')';
  }
...
function getParams(index) {
  if (this._params === undefined) {
    this._params = _.map(_.filter(this.parsed.tags, ['title', 'param']), function(tag) {
      var defaultValue = _.get(tag, 'default'),
          desc = util.format(_.get(tag, 'description')),
          name = _.get(tag, 'name') || '',
          type = reduceParamType(_.get(tag, 'type'));
...
function getReturns() {
  ...
  return tag ? [_.get(tag, 'type.name', '*'), _.get(tag, 'description', '')] : [];
}
 ...
 *
 * @memberOf Entry
 * @returns {string} The entry's `since` data.
 */
function getSince() {
